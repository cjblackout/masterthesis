...
def preprocess_data(file_path:str, columns:list, plot: bool=False) -> pd.DataFrame:
    ...
    try:
        dtypes = {'label': int, 'num_pixels': int, 'cX': int, 'cY': int, 'R': float, 'G': float, 'B': float, 'ground_truth': float, 'predicted_value': int}
        usecols = ['label', 'num_pixels', 'cX', 'cY', 'R', 'G', 'B', 'ground_truth', 'predicted_value']
        df_output = pd.read_csv(file_path, header=1, names=columns, dtype=dtypes, usecols=usecols)

        df_output.dropna(inplace=True)

        df_output = df_output.astype({'label': int, 'num_pixels': int, 'cX': int, 'cY': int, 'R': float, 'G': float, 'B': float, 'ground_truth': float, 'predicted_value': int})
